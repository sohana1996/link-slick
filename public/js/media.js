...
var openEditModal = function (urlId) {
    ...
    var source_id = form.find('select[name="source_id"]');
    ...
    $.ajax({
        url: apiUrl + '/get/single/media/' + urlId,
        method: 'GET',
        success: function (response) {
            var res = JSON.parse(response);
            if (res.status === 2000) {
                ...
                source_id.val(res.data[0].source_id);
